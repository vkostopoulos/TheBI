    ...
    public class ValidateCollectionResult
    {
        ...
        public ExtentDetails FirstExtentDetails
        {
            get
            {
                if (_firstExtentDetails == null)
                    ...
                    {
                        var match = Regex.Match(_response["result"].AsString,
                            @"first extent:\n(?<details>(    .+\n)+)");
        ...
        public bool IsValid
        {
            get
            {
                ...
                if (_response.Contains("errors") && !_response.Contains("valid"))
            ...
            get
            {
                if (_keysPerIndex == null)
                {
                    ...
                    if (_response.Contains("result") && !_response.Contains("keysPerIndex"))
                    {
                        ...
                        var indexStrings = match.Groups["value"].Value
                            .Split(new[] {'\n'}, StringSplitOptions.RemoveEmptyEntries);
                        foreach (var indexString in indexStrings)
                        {
                            var trimmedIndexString =
                                indexString.Substring(4 + prefixLength); // lines start with 4 blanks
        ...
        /// </summary>
        public string ResultString => _response["result"].AsString;
        ...
        {
            private readonly BsonDocument _detailsDocument;
